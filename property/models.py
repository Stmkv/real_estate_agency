...
class Flat(models.Model):
    ...
    who_liked = models.ManyToManyField(
        User, verbose_name="Кто лайкнул", blank=True, related_name="liked"
    )
...
class Owner(models.Model):
    full_name = models.CharField(
        "ФИО владельца", max_length=200, db_index=True, blank=True, null=True
    )
    pure_phone = PhoneNumberField(
        verbose_name="Нормализованный номер владельца", db_index=True, blank=True, null=True
    )
